...
Vue's reactivity system buffers all effects and flushes them asynchronously to avoid unnecessary duplicate invocation when there are many state mutations happening in the same "tick". Except on initial run,the effect is trigger synchronously and not be buffered. Internally, a component's `update` function is also a watched effect. When a user effect is queued, it is by default invoked **before** all component `update` effects:
